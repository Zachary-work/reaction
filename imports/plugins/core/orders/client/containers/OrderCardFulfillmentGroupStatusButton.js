...
function OrderCardFulfillmentGroupStatusButton({ fulfillmentGroup, order }) {
  ...
  //Reaction.hasPermission(["reaction-orders", "order/fulfillment"], Reaction.getUserId(), Reaction.getShopId());
  ...
  const canUpdateFulfillmentStatus = true;
  //(fulfillmentGroup.status !== "coreOrderWorkflow/canceled");
  const options = [
    {
